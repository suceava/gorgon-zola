...
export function InventoryManager() {
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-4 space-y-4">
      ...
      <p className="text-sm text-gray-400">Add items you have available for crafting.</p>
        ...
        <ul className="space-y-2">
          {inventory.map((item) => (
            <li key={item.itemId} className="flex items-center justify-between bg-gray-700 rounded px-3 py-2">
              ...
              <button onClick={() => removeItem(item.itemId)} className="text-red-400 hover:text-red-300 text-sm">
                Remove
              </button>
            </li>
          ))}
