...
export function ItemSearch() {
  ...
  return (
    <div className="space-y-4">
      ...
      {error && (
        <p className="text-red-400">Failed to load items: {(error as Error).message}</p>
      )}
          ...
          {items.map((item) => (
            ...
            >
              <div>
                ...
                {item.isCrafted && (
                  <span className="ml-2 text-xs px-1.5 py-0.5 bg-amber-900/50 text-amber-400 rounded">
                    Crafted
                  </span>
                )}
              </div>
              <span className="text-gray-400 text-sm">
                {item.value.toLocaleString()} councils
              </span>
            </div>
          ))}
        </div>
